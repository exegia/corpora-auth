...
Package manager is **bun** (pinned in root `package.json`). Node is still required — vitest and tsc shims spawn it, and CI pins it through `.github/actions/setup` because jsdom reads its version, not bun’s.
...
- **`bun pm pack` does not apply `publishConfig` field overrides** (pnpm did). Packing `@exegia/use-auth` naively yields a tarball whose `package.json` still points at `./src/index.ts`. `scripts/pack.sh` and `scripts/publish.sh` both apply it, via `apply_publish_config` in `scripts/lib.sh` — don't bypass them.
...
- **bun `--filter` doesn't carry flags through to the tauri CLI.** Run it from the app directory (`cd examples/tauri-app && bun run tauri dev --config …`).
...
`.github/WORKFLOW.md` is the full description; the shape is corpora-ui's, ported here in one migration (2026-08-01) that replaced the `dev → next → main` train and its five workflows:

```
<type>/<slug> ──PR──> release/vX.Y.Z ──PR──> main ──> crates.io + npm + tag
```

Everything CI does is a one-line `make` target, so every step is reproducible locally. The pipeline logic lives in `scripts/{version,publish,release}.sh`; `.github/workflows/*.yml` only wires triggers, permissions and tokens to them.

Three things about it that are specific to this repo:
...
The org-level allowed-actions policy on `exegia` (`selected`, one disallowed action kills a whole workflow at startup with no logs) now admits `dtolnay/rust-toolchain`, `Swatinem/rust-cache`, `supabase/setup-cli` and `oven-sh/setup-bun`, plus everything GitHub-owned and verified-creator. Read it with `gh api /orgs/exegia/actions/permissions/selected-actions` before adding a new action to any workflow — an unlisted one fails invisibly, and `gh run view --log-failed` shows nothing.
