...
| --- | --- |
| `allow-reset-password-for-email` | `resetPasswordForEmail`, `<ForgotPasswordForm />` |
| `allow-update-user` | `updateUser`, `<UpdatePasswordForm />`, `<OnboardingFlow />` |

`<OnboardingFlow />` needs `allow-update-user` because it persists step progress in `user_metadata` so an abandoned flow resumes on the next launch.
