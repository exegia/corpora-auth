...
<ParamField path="signInWithOAuth({ provider, scopes? })" type="Promise<Session>">
  Opens the system browser and resolves when the loopback round-trip completes. See [OAuth](/plugin/oauth).
</ParamField>
...
<ParamField path="linkIdentity({ provider, scopes? })" type="Promise<Identity[]>">
  Attaches a provider identity to the current account via the system browser. Needs `allow-link-identity` and `enable_manual_linking` on the project.
</ParamField>
