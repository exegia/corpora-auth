...
| Linux | None | No platform authenticator. `getPasskeyCapability()` reports it honestly and the UI kit blocks hide themselves |
...
```tsx
import { PasskeySignIn, PasskeyManager } from "@exegia/auth-ui";

// Sign-in screen — renders nothing when the device can't prompt
<PasskeySignIn onSignedIn={() => navigate("/app")} />;
...
- **Deleting the last passkey is not blocked server-side.** `<PasskeyManager />` warns before it happens; do the same in a custom UI, and keep another sign-in method on every account.
