...
<Note>
`message` is written for developers. For user-facing copy, use `@exegia/auth-ui` — every block renders resolved messages and accepts per-kind overrides through `errorMessages`.
</Note>
...
- `cancelOAuthFlow()` aborts a browser round-trip; the pending `signInWithOAuth()` call rejects with `oauthFlowInterrupted`, which the UI kit treats as a return to idle rather than a failure.
