...
- A GitHub token with `read:packages` to install the npm packages from GitHub Packages
...
Both npm packages live on GitHub Packages, so point the `@exegia` scope there:
...
The fastest path is a UI kit block:
