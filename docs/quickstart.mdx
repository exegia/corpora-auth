...
`@exegia/use-auth` (the React hooks) lives on the public npm registry; the bindings package lives on GitHub Packages. Install the hooks first, then point the `@exegia` scope at GitHub Packages for the bindings:

```bash
bun add @exegia/use-auth        # optional: the React hooks (public npm registry)
```
