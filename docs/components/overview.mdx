---
title: "Overview"
description: "@exegia/auth-ui: React hooks over the plugin bindings, with no UI of their own."
---

`@exegia/auth-ui` is a set of React hooks that talk to the plugin bindings directly. There is no provider or context to mount, no components, and no stylesheet — the package renders nothing, so every pixel of your auth screens stays yours.
...
<Card title="Hooks reference" icon="anchor" href="/components/hooks">
  `useSession`, `useAuth`, `useOnboarding`, `useOnboardingFlow`, `useIdentities`, `usePasskeys`
</Card>
